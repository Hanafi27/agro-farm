...
function filterTable() {
    ...
    rows.forEach(row => {
        ...
        if (show) {
            ...
            if (status === 'realized') realizedCount++;
        } else {
    ...
    document.getElementById('realizedCount').textContent = realizedCount;
}
...
function deleteItem(id) {
    if (confirm('Yakin ingin menghapus riwayat pengajuan ini?')) {
        ...
        fetch(`{{ route('admin.pengajuan-dana.delete-history', ':id') }}`.replace(':id', id), {
            method: 'DELETE',
            headers: {
...
function bulkDelete() {
    ...
    if (confirm(`Yakin ingin menghapus ${selectedIds.length} riwayat pengajuan yang dipilih?`)) {
        ...
        fetch('{{ route("admin.pengajuan-dana.bulk-delete-history") }}', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                'Content-Type': 'application/json',
